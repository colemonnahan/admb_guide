## Me playing with the hybrid algorithm in R. Based off the Brooks paper.
...
grad.U <- function(q){
    a <- q[1]; b <- q[2]
    ypred <- a*xobs+b
    dadf <- length(yobs)/2/sum((ypred-yobs)^2)*sum(2*xobs*(ypred-yobs))
...
grad.U2 <- function(q){
    a <- q[1]; b <- q[2]
    h <- .0001
    dadf <- (U(c(a+h,b))-U(c(a,b)))/h
    dbdf <- (U(c(a,b+h))-U(c(a,b)))/h
    return(c(dadf, dbdf))
}
grad.U(c(3,3))
grad.U2(c(3,3))
...
simple.hy1 <- HMC.simple(.1, 10, c(2,3))
contour(a.seq, b.seq, NLL)
points(a.mle, b.mle, col='red', pch=16)
n <- nrow(simple.hy1$leapfrog)
with(simple.hy1, arrows(leapfrog[-1,1], leapfrog[-1,2],
                        leapfrog[-n,1], leapfrog[-n,2]))
