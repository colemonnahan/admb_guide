...
%% This is a knitr file for creating the ADMB Bayesian guide. We originally
%% did it in knitr to have it be reproducible but in the end I don't think
%% it makes sense to run everything each time we recompile. Instead, simply
%% run the run_examples.R script and it will run everything and recreate
%% the plots (we think!). We could strip out knitr in future versions, but
%% for now it's left this way. NOTE: I had to knit the Rnw file and then
%% run BibTex manually on the produced .TeX file. Couldn't find a way to do
%% it within AUCTeX. CCM 10/12/2014.
...
%% Turned this off: 10/12/2014; CCM
...
technique that is not built in to ADMB. It requires a more
detailed understanding of the underlying output file structure
of ADMB. The steps to do this can be found in a
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{write.admb.cov} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{cov.user}\hlstd{,} \hlkwc{model.path}\hlstd{=}\hlkwd{getwd}\hlstd{())\{}
    \hlstd{temp} \hlkwb{<-} \hlkwd{file.exists}\hlstd{(}\hlkwd{paste0}\hlstd{(model.path,} \hlstr{"/admodel.cov"}\hlstd{))}
    \hlkwa{if}\hlstd{(}\hlopt{!}\hlstd{temp)} \hlkwd{stop}\hlstd{(}\hlkwd{paste0}\hlstd{(}\hlstr{"Couldn't find file "}\hlstd{,model.path,} \hlstr{"/admodel.cov"}\hlstd{))}
    \hlstd{temp} \hlkwb{<-} \hlkwd{file.copy}\hlstd{(}\hlkwc{from}\hlstd{=}\hlkwd{paste0}\hlstd{(model.path,} \hlstr{"/admodel.cov"}\hlstd{),}
                      \hlkwc{to}\hlstd{=}\hlkwd{paste0}\hlstd{(model.path,} \hlstr{"/admodel_original.cov"}\hlstd{))}
    \hlstd{wd.old} \hlkwb{<-} \hlkwd{getwd}\hlstd{()}
    \hlkwd{on.exit}\hlstd{(}\hlkwd{setwd}\hlstd{(wd.old))}
    \hlkwd{setwd}\hlstd{(model.path)}
    \hlcom{## Read in the output files}
    \hlstd{results} \hlkwb{<-} \hlkwd{get.admb.cov}\hlstd{()}
    \hlstd{scale} \hlkwb{<-} \hlstd{results}\hlopt{$}\hlstd{scale}
    \hlstd{num.pars} \hlkwb{<-} \hlstd{results}\hlopt{$}\hlstd{num.pars}
    \hlkwa{if}\hlstd{(}\hlkwd{NROW}\hlstd{(cov.user)} \hlopt{!=} \hlstd{num.pars)}
        \hlkwd{stop}\hlstd{(}\hlkwd{paste0}\hlstd{(}\hlstr{"Invalid size of covariance matrix, should be: "}\hlstd{, num.pars,}
                   \hlstr{"instead of "}\hlstd{,}\hlkwd{NROW}\hlstd{(cov.user)))}
    \hlstd{cov.unbounded} \hlkwb{<-} \hlstd{cov.user}\hlopt{/}\hlstd{(scale} \hlopt{%o%} \hlstd{scale)}
    \hlcom{## Write it back to file}
    \hlstd{file.new} \hlkwb{<-} \hlkwd{file}\hlstd{(}\hlkwd{paste0}\hlstd{(model.path,} \hlstr{"/admodel.cov"}\hlstd{),}\hlstr{"wb"}\hlstd{)}
    \hlkwd{on.exit}\hlstd{(}\hlkwd{close}\hlstd{(file.new))}
    \hlkwd{writeBin}\hlstd{(}\hlkwd{as.integer}\hlstd{(num.pars),} \hlkwc{con}\hlstd{=file.new)}
    \hlkwd{writeBin}\hlstd{(}\hlkwd{as.vector}\hlstd{(}\hlkwd{as.numeric}\hlstd{(cov.unbounded)),} \hlkwc{con}\hlstd{=file.new)}
    \hlkwd{writeBin}\hlstd{(}\hlkwd{as.integer}\hlstd{(results}\hlopt{$}\hlstd{hybrid_bounded_flag),} \hlkwc{con}\hlstd{=file.new)}
    \hlkwd{writeBin}\hlstd{(}\hlkwd{as.vector}\hlstd{(scale),} \hlkwc{con}\hlstd{=file.new)}
\hlstd{\}}
\end{alltt}
\end{kframe}
\end{knitrout}
