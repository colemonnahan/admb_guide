...
be merged directly to the \texttt{.Rnw} file (\texttt{knitr}), and
...
parameter estimates and credible intervals) can be accomplished. In
...
taken from it could lead to invalid inference. \textbf{The
  ...
  samples from a MCMC chain.} The R package CODA is
...
When running MCMC in ADMB, users are \textbf{not} forced
...
\item Pull results into R, Excel or other program to ensure the
  ...
  tools using, for example, the CODA package.
...
chain has ``converged'' (roughly speaking this means it is producing
independent samples from the posterior). There are a couple key diagnostics to
...
decades, is simple to implement, and is used widely.
...
the chain will not explore the parameter space. The optimal acceptance
