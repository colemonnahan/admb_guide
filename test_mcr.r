...
test_mcr <- function(model.path, 
  ...
  ## Add mcr if desired
  if(!is.null(mcr)) cmd <- paste(paste0('./', model.name), '-mcr', mcr)
  ...
  system(paste(paste0('./', model.name), "-mceval -noest -nohess"), ignore.stdout=T)
  ...
  write.csv(results$mcmc, file = 'par_draws.csv')
  return(results)
}
